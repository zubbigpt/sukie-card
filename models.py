...
class LoyaltyCard(Base):
    ...
    stamps           = Column(Integer, default=0)   # sellos actuales en la tarjeta vigente
    total_stamps     = Column(Integer, default=0)   # sellos totales acumulados en toda la vida
    rewards_redeemed = Column(Integer, default=0)   # premios canjeados (histórico)
    award_balance    = Column(Integer, default=0)   # premios disponibles pendientes de canjear
    created_at       = Column(DateTime(timezone=True), server_default=func.now())
    updated_at       = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class StampTransaction(Base):
    ...
    transaction_type = Column(String, default="stamp")  # stamp | redeem | adjust
